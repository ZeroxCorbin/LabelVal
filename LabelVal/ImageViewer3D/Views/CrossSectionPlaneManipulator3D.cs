...
namespace LabelVal.ImageViewer3D.Views
{
    public class CrossSectionPlaneManipulator3D : GroupModel3D
    {
        ...
        public Vector3 ConstrainDimensions
        {
            get { return (Vector3)GetValue(ConstrainDimensionsProperty); }
            set { SetValue(ConstrainDimensionsProperty, value); }
        }

        // Using a DependencyProperty as the backing store for ConstrainDimensions.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty ConstrainDimensionsProperty =
            DependencyProperty.Register("ConstrainDimensions", typeof(Vector3), typeof(CrossSectionPlaneManipulator3D), new PropertyMetadata(new Vector3(),
                (d, e) =>
                {
                    var model = d as CrossSectionPlaneManipulator3D;
                    model.UpdateScaling((float)model.CornerScale, (float)model.EdgeThicknessScale, (float)model.SizeScale, (Vector3)e.NewValue);
                }));
        ...
        public static readonly DependencyProperty SizeScaleProperty =
            DependencyProperty.Register("SizeScale", typeof(double), typeof(CrossSectionPlaneManipulator3D), new PropertyMetadata(1.0,
                ...
                {
                    var model = d as CrossSectionPlaneManipulator3D;
                    model.UpdateScaling((float)model.CornerScale, (float)model.EdgeThicknessScale, (float)(double)e.NewValue, model.ConstrainDimensions);
                }));
        ...
        public static readonly DependencyProperty CornerScaleProperty =
            DependencyProperty.Register("CornerScale", typeof(double), typeof(CrossSectionPlaneManipulator3D), new PropertyMetadata(1.0,
                ...
                {
                    var model = d as CrossSectionPlaneManipulator3D;
                    model.UpdateScaling((float)(double)e.NewValue, (float)model.EdgeThicknessScale, (float)model.SizeScale, model.ConstrainDimensions);
                }));
        ...
        private void OnEdgeMouse3DMove(object sender, RoutedEventArgs e)
        {
            if (isCaptured && e is Mouse3DEventArgs arg && arg.Viewport == viewport)
            {
                ...
                if (newHit.HasValue)
                {
                    ...
                    {
                        // Project the offset onto the ConstrainAxis
                        var axis = Vector3.Normalize(ConstrainAxis.Value);
                        offset = Vector3.Dot(offset, axis) * axis;
                    }
        ...
        private void RotateTrackball(Point p1, Point p2, Vector3 rotateAround)
        {
            ...
            if (ConstrainAxis.HasValue)
            {
                ...
                currentRotation *= Matrix.RotationAxis(Vector3.Normalize(ConstrainAxis.Value), (float)(angle * RotationSensitivity * 5));
                UpdateTransform();
            }
            else
            {
                ...
                // Create the transform
                currentRotation *= Matrix.RotationAxis(Vector3.Normalize(axis), (float)(angle * RotationSensitivity * 5));
                UpdateTransform();
            }
