...
public partial class Sector : ObservableObject, ISector, IDisposable
{
    ...
    public string DecodeID { get; }

    public Sector(JObject report, string reportId, string decodeId, JObject template, ApplicationStandards standard, GS1Tables table, string version)
    {
        Version = version;
        DesiredApplicationStandard = standard;
        ...
        if (string.IsNullOrEmpty(decodeId))
         name = $"SymbologyTool_{reportId}";
        else
            name = $"SymbologyTool_{decodeId}";

        Template = new SectorTemplate(report, reportId, template, name, version);
        Report = new SectorReport(report, reportId, decodeId, Template, table);
    ...
    private void UpdateFocusedParameters()
    {
        ...
        foreach (var parameter in lst)
        {
            if (!FocusedParameters.Any(p => p.Parameter == parameter))
            {
                ...
                {
                    ParameterHandling.AddParameter(parameter, Report.Symbology, FocusedParameters, Report.Original, Template.Original, ReportID, DecodeID);
                }
            }
        }
